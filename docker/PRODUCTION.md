...
# Путь к index.html внутри контейнера сервера. Файл встроен в образ при сборке (Server/Dockerfile).
...
2. **Бэкенд:** в окружении сервера заданы `BASE_URL` (например `https://top-pet.ru`) и `SPA_INDEX_PATH=/app/static/index.html`. Файл `index.html` встроен в образ сервера при сборке (мультистейдж в `Server/Dockerfile`), монтировать `client/build` не нужно.
...
После обновления кода пересоберите образы. Образ сервера при сборке сам собирает клиент и встраивает `index.html` для превью в соцсетях.
...
3. **Файл index.html**  
   Бэкенд читает `index.html` по пути `SPA_INDEX_PATH`. Файл встроен в образ сервера при сборке (`Server/Dockerfile` собирает клиент и копирует `index.html` в `/app/static/`). Монтировать `client/build` не нужно. Если при сборке сервера использовался контекст не из корня репозитория, пересоберите из корня: `docker build -f Server/Dockerfile .` в каталоге репозитория.
