...
После обновления кода пересоберите образы. В образе сервера уже встроен `Server/static/index.html` для превью в соцсетях.
...
Бэкенд собирается **только из папки Server**: контекст сборки — `Server/`, Dockerfile — `Server/Dockerfile`. Файл `Server/static/index.html` (минимальный шаблон для og/twitter-превью) встроен в образ, отдельно собирать клиент не нужно.

В Coolify укажите для бэкенда:

- **Build context / Base directory:** `Server` (или путь к папке Server в клоне)
- **Dockerfile:** `Dockerfile` (внутри папки Server)

Переменная окружения: `SPA_INDEX_PATH=/app/static/index.html`.
...
3. **Файл index.html**  
   Бэкенд читает `index.html` по пути `SPA_INDEX_PATH`. Файл встроен в образ из `Server/static/index.html` при сборке (контекст — папка `Server/`). Монтировать `client/build` не нужно.
...
4. **Кэш Telegram**  
   Если раньше по этой ссылке превью было пустым или с ошибкой, Telegram мог закэшировать. Варианты: отправить ссылку боту [@webpagebot](https://t.me/webpagebot) для обновления превью или добавить к URL параметр, например `?v=2`, и отправить уже новую ссылку.
