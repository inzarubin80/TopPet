package config

import (
	"fmt"
	"os"
	"strings"

	"golang.org/x/oauth2"
	"golang.org/x/oauth2/yandex"
	authinterface "toppet/server/internal/app/authinterface"
	providerUserData "toppet/server/internal/app/clients/provider_user_data"
	"toppet/server/internal/app/icons"
)

// yandexOAuthScopesFromEnv returns scopes for Yandex OAuth authorize URL.
// If YANDEX_OAUTH_SCOPES is unset or empty, returns nil: /authorize must omit scope so that
// Yandex issues a token with permissions from the app registration (avoids invalid_scope
// when the code requests rights not enabled for the app).
// If set, use a space-separated list (must match Yandex OAuth console exactly; phone is login:default_phone, not login:phone).
func yandexOAuthScopesFromEnv() []string {
	raw := strings.TrimSpace(os.Getenv("YANDEX_OAUTH_SCOPES"))
	if raw == "" {
		return nil
	}
	var out []string
	for _, p := range strings.Fields(raw) {
		if p != "" {
			out = append(out, p)
		}
	}
	if len(out) == 0 {
		return nil
	}
	return out
}

// LoadOAuthProviders загружает конфигурацию OAuth провайдеров из переменных окружения.
// Ошибочная конфигурация одного провайдера (например, задан CLIENT_ID без секрета) не отключает остальные:
// такой провайдер пропускается с предупреждением в stdout.
func LoadOAuthProviders() authinterface.MapProviderOauthConf {
	providers := make(authinterface.MapProviderOauthConf)
	apiRoot := os.Getenv("API_ROOT")
	if apiRoot == "" {
		apiRoot = "http://localhost:8080"
	}

	loadYandex(providers, apiRoot)
	loadGoogle(providers, apiRoot)
	loadVK(providers, apiRoot)

	return providers
}

func loadYandex(providers authinterface.MapProviderOauthConf, apiRoot string) {
	clientID := os.Getenv("CLIENT_ID_YANDEX")
	if clientID == "" {
		return
	}
	clientSecret := os.Getenv("CLIENT_SECRET_YANDEX")
	if clientSecret == "" {
		fmt.Printf("Warning: OAuth provider yandex skipped: CLIENT_SECRET_YANDEX is required when CLIENT_ID_YANDEX is set\n")
		return
	}

	yandexScopes := yandexOAuthScopesFromEnv()
	providers["yandex"] = &authinterface.ProviderOauthConf{
		Oauth2Config: &oauth2.Config{
			ClientID:     clientID,
			ClientSecret: clientSecret,
			RedirectURL:  fmt.Sprintf("%s/api/auth/callback?provider=yandex", apiRoot),
			Scopes:       yandexScopes,
			Endpoint:     yandex.Endpoint,
		},
		UrlUserData: "https://login.yandex.ru/info?format=json",
		IconSVG:     icons.GetProviderIcon("yandex"),
		DisplayName: "Яндекс",
		ProviderUserData: providerUserData.NewProviderUserData(
			"https://login.yandex.ru/info?format=json",
			&oauth2.Config{
				ClientID:     clientID,
				ClientSecret: clientSecret,
				RedirectURL:  fmt.Sprintf("%s/api/auth/callback?provider=yandex", apiRoot),
				Scopes:       yandexScopes,
				Endpoint:     yandex.Endpoint,
			},
			"yandex",
		),
	}
}

func loadGoogle(providers authinterface.MapProviderOauthConf, apiRoot string) {
	clientID := os.Getenv("CLIENT_ID_GOOGLE")
	if clientID == "" {
		return
	}
	clientSecret := os.Getenv("CLIENT_SECRET_GOOGLE")
	if clientSecret == "" {
		fmt.Printf("Warning: OAuth provider google skipped: CLIENT_SECRET_GOOGLE is required when CLIENT_ID_GOOGLE is set\n")
		return
	}

	providers["google"] = &authinterface.ProviderOauthConf{
		Oauth2Config: &oauth2.Config{
			ClientID:     clientID,
			ClientSecret: clientSecret,
			RedirectURL:  fmt.Sprintf("%s/api/auth/callback?provider=google", apiRoot),
			Scopes:       []string{"openid", "email", "profile"},
			Endpoint: oauth2.Endpoint{
				AuthURL:  "https://accounts.google.com/o/oauth2/auth",
				TokenURL: "https://oauth2.googleapis.com/token",
			},
		},
		UrlUserData: "https://www.googleapis.com/oauth2/v2/userinfo",
		IconSVG:     icons.GetProviderIcon("google"),
		DisplayName: "Google",
		ProviderUserData: providerUserData.NewProviderUserData(
			"https://www.googleapis.com/oauth2/v2/userinfo",
			&oauth2.Config{
				ClientID:     clientID,
				ClientSecret: clientSecret,
				RedirectURL:  fmt.Sprintf("%s/api/auth/callback?provider=google", apiRoot),
				Scopes:       []string{"openid", "email", "profile"},
				Endpoint: oauth2.Endpoint{
					AuthURL:  "https://accounts.google.com/o/oauth2/auth",
					TokenURL: "https://oauth2.googleapis.com/token",
				},
			},
			"google",
		),
	}
}

func loadVK(providers authinterface.MapProviderOauthConf, apiRoot string) {
	clientID := os.Getenv("CLIENT_ID_VK")
	if clientID == "" {
		return
	}
	clientSecret := os.Getenv("CLIENT_SECRET_VK")
	if clientSecret == "" {
		fmt.Printf("Warning: OAuth provider vk skipped: CLIENT_SECRET_VK is required when CLIENT_ID_VK is set\n")
		return
	}

	providers["vk"] = &authinterface.ProviderOauthConf{
		Oauth2Config: &oauth2.Config{
			ClientID:     clientID,
			ClientSecret: clientSecret,
			RedirectURL:  fmt.Sprintf("%s/api/auth/callback?provider=vk", apiRoot),
			Scopes:       []string{"email"},
			Endpoint: oauth2.Endpoint{
				AuthURL:  "https://oauth.vk.com/authorize",
				TokenURL: "https://oauth.vk.com/access_token",
			},
		},
		UrlUserData: "https://api.vk.com/method/users.get?fields=photo_200&v=5.131",
		IconSVG:     icons.GetProviderIcon("vk"),
		DisplayName: "VK",
		ProviderUserData: providerUserData.NewProviderUserData(
			"https://api.vk.com/method/users.get?fields=photo_200&v=5.131",
			&oauth2.Config{
				ClientID:     clientID,
				ClientSecret: clientSecret,
				RedirectURL:  fmt.Sprintf("%s/api/auth/callback?provider=vk", apiRoot),
				Scopes:       []string{"email"},
				Endpoint: oauth2.Endpoint{
					AuthURL:  "https://oauth.vk.com/authorize",
					TokenURL: "https://oauth.vk.com/access_token",
				},
			},
			"vk",
		),
	}
}
